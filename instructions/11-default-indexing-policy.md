...
1. **[Indexing Policy]** セクションで、既定のインデックス作成ポリシーを確認します。
...
1. **[Save]** を選択してご自身の変更を保存します。

1. **[New SQL Query]** を選択します。

1. エディター領域のコンテンツを削除します。

1. **name** が **HL Headset** と同じであるすべてのドキュメントを返す新しい SQL クエリを作成します。
...
1. **price** が **\$3,000** を超えるすべてのドキュメントを返す新しい SQL クエリを作成します。
